...
def layout_participantes(i):
    return [[sg.Text('Participante: '), sg.InputText(key=('-PTCP-', i)), ]]
...
layout_coluna = [
    [sg.Text('Participante: '), sg.InputText(key=('-PTCP-', 0)),
     sg.Button(enable_events=True, image_data=PLUS_ICO, key='-add-')]
]


layout = [
    [(sg.Text('Insira todos os participantes: '))],
    [sg.Column(layout_coluna, key='-Coluna-')],
    [sg.Submit(button_text='Inserir'), sg.Cancel(button_text='Cancel')],
]


window_nomes = Window('Insira os nomes', layout, modal=True)


i = 1
while True:
    event, values = window_nomes.read()
    if event in (sg.WIN_CLOSED, 'Exit', 'Cancel'):
        break
    elif event == '-add-':
        if i < 5:
            window_nomes.extend_layout(
                window_nomes['-Coluna-'], layout_participantes(i))
            i += 1
        print(event, values)
event, values = window_nomes.read()
window_nomes.close
